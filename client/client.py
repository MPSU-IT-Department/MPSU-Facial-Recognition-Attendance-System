...
except Exception as deepface_error:  # pragma: no cover - import guard
    DeepFace = None
...
# Server URL and API key are centralized in server.py
...
FACE_ENCODINGS_CACHE = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'cache', 'face_encodings.pkl')
)
FACE_ENCODINGS_ENDPOINT = f"{SERVER_URL}/api/face-encodings"

MPSU_LOGO_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), 'MPSU.png')
)

CLASS_STATE_CACHE_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'cache', 'class_state.json')
)
...
TIME_RANGE_PATTERN = re.compile(r'(\d{1,2}:\d{2}\s*(?:[AaPp][Mm])?)\s*-\s*(\d{1,2}:\d{2}\s*(?:[AaPp][Mm])?)', re.IGNORECASE)
_ROOM_OPTIONS_CACHE = {
    'rooms': DEFAULT_ROOM_OPTIONS.copy(),
    'timestamp': 0,
...
def compute_class_timeout_info(class_obj, target_date=None):
    ...
    if window:
        return {
            'duration_minutes': max(1, window['duration_minutes']),
            'window': window,
            'source': 'schedule',
        }
    return {
...
def fetch_active_sessions():
    ...
    try:
        response = requests.get(
            f'{SERVER_URL}/api/sessions/active',
            headers=HEADERS,
            verify=False,
            timeout=10,
        )
    ...
    for session in sessions:
        ...
        normalized[class_id] = {
            ...
            'class_code': session.get('class_code') or '',
            'description': session.get('description') or '',
            'instructor_id': session.get('instructor_id'),
            'view_lock_owner': session.get('view_lock_owner'),
        }
...
def _request_session_view_lock(session_id, locker_id, action, force=False):
    ...
    try:
        response = requests.post(
            f"{SERVER_URL}/api/sessions/{session_id}/view-lock",
...
def acquire_remote_view_lock(class_id, session_id):
    ...
    success, data_or_error, status = _request_session_view_lock(
        session_id,
        CLIENT_INSTANCE_ID,
        action='lock',
    )
...
def release_remote_view_lock(class_id, session_id=None, force=False):
    ...
    # If unlock failed because another kiosk owns it, leave the cache untouched
    return False, data_or_error
...
def _handle_class_timeout(class_id):
    ...
    # Check if we've already shown a timeout message for this class
    ...
    # Mark that we've shown the timeout message
    _timeout_message_shown.add(class_id)
    
    # Clear the flag after a delay to allow re-showing if needed (e.g., if class restarts)
    def clear_timeout_flag():
        _timeout_message_shown.discard(class_id)
    if root:
        root.after(5000, clear_timeout_flag)  # Clear after 5 seconds
...
def sync_active_sessions_from_server(active_sessions, classes_by_id):
    ...
    for class_id, session in active_sessions.items():
        ...
        schedule_class_timeout(
            class_id,
            start_time=session.get('start_time'),
            class_payload=(classes_by_id or {}).get(class_id),
            timeout_deadline=session.get('timeout_deadline') or session.get('scheduled_end_time'),
        )

    # Drop stale instructor bindings
    ...
    for class_id, session in active_sessions.items():
        ...
        class_instructor_assignments[class_id] = {
            'id': instructor_id,
...
def room_has_active_session(room_value, room_session_map, exempt_class_id=None):
    ...
    for session in sessions:
        occupant_class_id = _coerce_int(
            session.get('class_id')
            or session.get('classId')
            or session.get('id')
        )
        ...
        if occupant_class_id is not None:
            ...
            if occupant_class_id not in ongoing_classes:
                # Only treat rooms as occupied if the class is actively running.
                continue
        else:
            # Unknown owner, treat as free until kiosk receives more info.
            continue
...
def normalize_class_payload(raw_class):
    ...
    normalized = {
        'id': _value('id', 'class_id'),
        'class_code': _value('class_code', 'classCode') or '',
        'description': _value('description') or '',
        'schedule': _value('schedule') or '',
        'room_number': _value('room_number', 'roomNumber') or '',
        'instructor_id': _value('instructor_id', 'instructorId'),
        'substitute_instructor_id': _value('substitute_instructor_id', 'substituteInstructorId'),
        'instructor_name': _value('instructor_name', 'instructorName') or '',
        'substitute_instructor_name': _value('substitute_instructor_name', 'substituteInstructorName') or '',
    }

    # Preserve commonly accessed camelCase keys for downstream API calls
    ...
    # Pass through any additional metadata so other features keep working
...
class InstructorLoginScanner:
    ...
    def __init__(self, parent, on_success, on_closed=None):
        if DeepFace is None:
            raise RuntimeError(
                f"DeepFace library is unavailable: {_DEEPFACE_IMPORT_ERROR}"
            )
        ...
        try:
            ...
            if not cache_updated and not os.path.exists(FACE_ENCODINGS_CACHE):
                raise RuntimeError(
                    "Unable to download face encoding cache from the server."
                )

            self._load_embeddings()
            if not self.instructor_embeddings:
                raise RuntimeError(
                    "No instructor face data found. Run the embedding extractor before using facial login."
                )
        except Exception:
    ...
    def _try_download_cache_on_startup(self):
        ...
        try:
            response = requests.get(
                f"{SERVER_URL}/api/face-encodings/meta",
                headers=HEADERS,
                verify=False,
                timeout=3
            )
    ...
    def _reload_embeddings(self):
        ...
        try:
            ...
            self._load_embeddings()
            self._update_cache_mtime()
            print(f"✅ Successfully reloaded {len(self.instructor_embeddings)} instructor embeddings")
        except Exception as e:
    ...
    def _load_embeddings(self):
        ...
        if not os.path.exists(FACE_ENCODINGS_CACHE):
            raise RuntimeError(
                f"Face encoding cache not found at {FACE_ENCODINGS_CACHE}."
            )
        ...
        self.instructor_embeddings = [
            self._normalize_embedding(np.array(embedding))
            for embedding in instructor_embeds
        ]
        ...
        while self.running:
            ...
            for rep in representations:
                ...
                if match:
                    ...
                    self.window.after(
                        0,
                        lambda i=instructor_id, n=name: self._handle_success(i, n),
                    )
                    break
    ...
    def _compare_embeddings(self, embedding):
        ...
        if best_idx >= 0 and min_distance < threshold:
            instructor_name = (
                self.instructor_names[best_idx]
                if best_idx < len(self.instructor_names)
                else "Instructor"
            )
            instructor_id = (
                self.instructor_ids[best_idx]
                if best_idx < len(self.instructor_ids)
                else None
            )
            if instructor_id:
...
AUTO_REFRESH_INTERVAL_MS = 5000  # Refresh class list every 5 seconds
SCANNER_SESSION_MONITOR_INTERVAL_MS = 4000  # Poll for remote session ends every 4 seconds
classes_auto_refresh_job = None
ongoing_classes = set()  # Track class IDs that are currently ongoing
class_rooms = {}  # Remember selected room per class_id for ongoing sessions
class_session_ids = {}  # Track active class_session IDs returned after instructor check-in
class_instructor_assignments = {}  # Track which instructor started each class
...
_timeout_message_shown = set()  # Track which classes have already shown timeout messages
class_card_widgets = {}
...
def _build_state_snapshot():
    ...
    return {
        'ended_classes_day': day.isoformat(),
        'ended_classes': sorted(int(cid) for cid in ended_classes),
        'ended_class_expirations': _serialize_datetime_map(ended_class_expirations),
        'ongoing_classes': sorted(int(cid) for cid in ongoing_classes),
        'class_rooms': _serialize_simple_map(class_rooms),
        'class_session_ids': _serialize_simple_map(class_session_ids),
        'class_instructor_assignments': _serialize_instructor_map(),
    }
...
def start_instructor_face_login(on_success=None):
    ...
    try:
        active_login_scanner = InstructorLoginScanner(
            parent,
            on_success=_dispatch_success,
            on_closed=_scanner_closed,
        )
    except Exception as exc:
        _scanner_closed()
...
def schedule_scanner_session_monitor(class_id, session_id):
    ...
    def _dispatch_check():
        ...
        session_mismatch = (
            ...
            and remote_session_id is not None
            and remote_session_id != expected_session_id
        )
    ...
    try:
        scanner_session_monitor_job = parent.after(
            SCANNER_SESSION_MONITOR_INTERVAL_MS,
            _dispatch_check,
        )
    except Exception:
        scanner_session_monitor_job = None
...
def determine_class_ui_state(cls, room_session_map):
    ...
    if is_ongoing:
        ...
        action_mode = 'view'
        # Allow operators to override remote locks while keeping local locks read-only.
        ...
        # Only block manual starts when the class already ended or its schedule window passed.
    ...
    return {
        ...
        'status_font': ("Arial", 22, "bold" if is_ongoing else "normal"),
        ...
        'room_is_occupied': room_is_occupied,
    ...
    if status_label:
        try:
            status_label.configure(
                ...
                text_color=state['status_color'],
                font=state['status_font'],
            )
        except Exception:
            pass
    ...
    if action_button:
        ...
        try:
            action_button.configure(
                ...
                fg_color=state['action']['fg_color'],
                hover_color=state['action']['hover_color'],
                command=command,
            )
        except Exception:
            pass
...
def show_today_classes():
    ...
    # Create a frame for the title to center it with MPSU green theme
    title_frame = ctk.CTkFrame(root, fg_color=("#f0f8f0", "#2d5a2d"))
    title_frame.pack(pady=20, fill="x")

    # Create a container for title only
    ...
    title_label = ctk.CTkLabel(
        title_column,
        text=f"Classes for {today_label}",
        font=("Arial", 26, "bold"),
        text_color=("#006400", "#90EE90"),
    )
    ...
    refresh_btn = ctk.CTkButton(
        ...
        command=show_today_classes,
    )
    refresh_btn.pack(side="right")

    # Fetch classes from API and normalize their shape
    try:
        response = requests.get(f'{SERVER_URL}/classes/api/list', headers=HEADERS, verify=False)
        if response.status_code != 200:
            error_label = ctk.CTkLabel(
                root,
                text=f"Failed to load classes: {response.status_code} - {response.text}",
                font=("Arial", 18)
            )
        ...
        if not todays_classes:
            no_classes_label = ctk.CTkLabel(
                root,
                text=f"No classes scheduled for {today_label}.",
                font=("Arial", 20, "bold"),
                text_color=("#006400", "#90EE90"),
            )
            ...
            scrollable_frame = ctk.CTkScrollableFrame(
                ...
                width=600,
                height=400,
                fg_color=("#f8fff8", "#2d5a2d"),
                scrollbar_button_color=("#228B22", "#32CD32"),
            )
            ...
            def room_is_currently_occupied(room_value, exempt_class_id=None, force_refresh=False, room_map_override=None):
                ...
                return room_has_active_session(
                    room_value,
                    active_map,
                    exempt_class_id=exempt_class_id,
                )
            ...
            def room_occupant_summary(room_value, exempt_class_id=None, force_refresh=False, room_map_override=None):
                ...
                for session in sessions:
                        ...
                        if instructor_id_int is not None:
                            ...
                            for cls_payload in class_metadata_by_id.values():
                                if cls_payload.get('id') == session.get('class_id'):
                                    instructor_name = (
                                        cls_payload.get('instructor_name')
                                        or cls_payload.get('instructorName')
                                    )
                                    break
                    ...
                    if (
                        occupant_class_id is not None
                        and exempt_class_id is not None
                        and occupant_class_id == exempt_class_id
                    ):
                        continue
            ...
            def confirm_room_override(room_value, exempt_class_id=None, room_map_override=None):
                summary = room_occupant_summary(
                    room_value,
                    exempt_class_id,
                    force_refresh=(room_map_override is None),
                    room_map_override=room_map_override,
                )
                message = (
                    f"Room {room_value} is currently marked as occupied.\n\n"
                    "Only continue if you are sure no instructor is already running a session "
                    "in this room or the kiosk was reset."
                )
                if summary:
                    message += f"\n\nCurrently running:\n{summary}"
            ...
            def _open_scanner_window(class_id, room_input, session_id=None):
                ...
                try:
                    ...
                    app = FacialRecognitionApp(
                        ...
                        room_number=room_input,
                        embedded=True,
                        on_exit=on_scanner_exit,
                        on_logout=on_scanner_logout,
                        acting_instructor_id=instructor_binding.get('id'),
                        acting_instructor_role=instructor_binding.get('role'),
                    )
                    ...
                    messagebox.showerror(
                        "Scanner Error",
                        f"Failed to open facial recognition scanner:\n{e}"
                    )
                    show_today_classes()
                    return False
            ...
            def start_facial_recognition_for_class_show_room_modal(class_id_for_modal):
                ...
                title_label = ctk.CTkLabel(
                    main_frame,
                    text="🏫 Room Verification Required",
                    font=("Arial", 28, "bold"),
                    text_color=("#006400", "#90EE90"),
                )
                ...
                selected_class = next(
                    (c for c in displayed_classes if c.get('id') == class_id_for_modal),
                    None
                )
                ...
                instruction_label = ctk.CTkLabel(
                    main_frame,
                    text="Please select the room number to start facial recognition:",
                    font=("Arial", 18),
                    text_color=("#2d5a2d", "#b8e6b8"),
                )
                ...
                for option in room_options:
                    occupied = room_is_currently_occupied(
                        option,
                        class_id_for_modal,
                        room_map_override=modal_room_map,
                    )
                ...
                dropdown = ctk.CTkOptionMenu(
                    ...
                    fg_color=("#228B22", "#32CD32"),
                    text_color="#ffffff",
                    button_color=("#006400", "#004d00"),
                    button_hover_color=("#004d00", "#003000"),
                    dropdown_fg_color=("#e8f5e8", "#1e4a1e"),
                    dropdown_hover_color=("#cdeacc", "#2f6a2f"),
                )
                ...
                hint_label = ctk.CTkLabel(
                    main_frame,
                    text="Rooms marked as Occupied are locked until that class finishes.",
                    font=("Arial", 16),
                    text_color=('#8B0000', '#f8d7da'),
                )
                hint_label.pack(pady=(0, 10))

                room_status_label = ctk.CTkLabel(
                    main_frame,
                    text="",
                    font=("Arial", 17),
                    text_color=('#8B0000', '#f5c6cb'),
                    justify="left",
                )
                ...
                def update_room_status(*_args):
                    ...
                    if entry:
                        ...
                        if room_is_currently_occupied(
                            entry['value'],
                            class_id_for_modal,
                            room_map_override=active_map,
                        ):
                            summary = room_occupant_summary(
                                entry['value'],
                                class_id_for_modal,
                ...
                def confirm_room():
                    ...
                    if room_is_currently_occupied(
                        ...
                        feedback_label.configure(
                            text="That room is currently in use. Please choose another room."
                        )
            ...
            def record_instructor_checkin(cls, instructor_id, room_input):
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Check-In Failed",
                        "Instructor identifier is invalid. Please rescan."
                    )
                    return None
                payload = {
                ...
                try:
                    response = requests.post(
                        f'{SERVER_URL}/api/checkin/instructor',
                        headers=JSON_HEADERS,
                        json=payload,
                        verify=False,
                        timeout=15,
                    ...
                    messagebox.showerror(
                        "Check-In Failed",
                        f"Unable to reach the server for instructor check-in.\n{exc}"
                    )
                    return None
                ...
                if response.status_code != 200:
                    messagebox.showerror(
                        "Check-In Failed",
                        f"Server rejected the instructor check-in (status {response.status_code}).\n{response.text}"
                    )
                    return None
                ...
                if not session_id:
                    messagebox.showerror(
                        "Check-In Failed",
                        "Server did not return an active class session ID."
                    )
                    return None

                return {
                    'session_id': session_id,
                    'scheduled_start_time': data.get('scheduled_start_time') or data.get('scheduledStartTime'),
                    'scheduled_end_time': data.get('scheduled_end_time') or data.get('scheduledEndTime'),
                    'timeout_deadline': data.get('timeout_deadline') or data.get('timeoutDeadline'),
                }
            ...
            def show_override_modal(parent, lock_owner=None, context="override"):
                ...
                if is_start_confirmation:
                    ...
                    message = (
                        "Are you sure you want to view this ongoing class session?\n\n"
                    ...
                    message = (
                        "Only continue if there is no instructor Present "
                        "or if the system has accidentally shutdown. Do you wish to continue?"
                    )
            ...
            def view_ongoing_class(cls):
                ...
                if not session_id:
                    messagebox.showerror(
                        "Session Missing",
                        "Unable to locate the active session for this class. Please start the class again."
                    )
                    ongoing_classes.discard(class_id)
                    class_rooms.pop(class_id, None)
                    class_session_ids.pop(class_id, None)
                ...
                if remote_lock_active:
                    ...
                    unlock_success, unlock_error = release_remote_view_lock(
                        class_id,
                        session_id,
                        force=True,
                    )
                    if not unlock_success:
                        messagebox.showerror(
                            "Override Failed",
                            f"Unable to free the session for viewing.\n{unlock_error or 'Unknown error.'}"
                        )
                ...
                if not allowed_roles:
                    messagebox.showerror(
                        ...
                        "Unable to verify the assigned instructor for this class."
                    )
                    return
                ...
                def _after_scan(instructor_id, instructor_name):
                    ...
                    if instructor_id_int is None:
                        messagebox.showerror(
                            "Instructor Verification",
                            "Unable to determine the instructor identity from the scan."
                        )
                        return
                    ...
                    if instructor_id_int not in allowed_roles:
                        messagebox.showerror(
                            "Instructor Verification",
                            "The recognized instructor is not assigned to this class."
                        )
                        return
                    ...
                    if not success:
                        ...
                        messagebox.showerror(
                            "Instructor Verification",
                            f"Unable to reserve the scanner for this session:\n{lock_error}"
                        )
                        return
                    ...
                    class_instructor_assignments[class_id] = {
                        'id': instructor_id_int,
                        'role': allowed_roles[instructor_id_int],
                    }
                    persist_class_state()
            ...
            def start_class_with_instructor_verification(cls):
                ...
                if not allowed_roles:
                    messagebox.showerror(
                        "Instructor Required",
                        "This class does not have an assigned instructor or substitute. Please assign one before starting the session."
                    )
                    return
                ...
                def _after_scan(instructor_id, instructor_name):
                    ...
                    try:
                        ...
                        messagebox.showerror(
                            ...
                            "Unable to validate the instructor assignment for this class."
                        )
                        return
                    ...
                    while True:
                        active_map = get_active_room_map(force_refresh=True)
                        if room_input and room_is_currently_occupied(
                            room_input,
                            class_id,
                            room_map_override=active_map,
                        ):
            ...
            for cls in displayed_classes:
                ...
                instructor_label = ctk.CTkLabel(
                    inline_frame,
                    text=f"Instructor: {instructor_name}",
                    font=("Arial", 20),
                    text_color=detail_color,
                )
                ...
                timeout_label = ctk.CTkLabel(
                    inline_frame,
                    text=state['timeout_text'],
                    font=("Arial", 18),
                    text_color=timeout_color
                )
                ...
                class_card_widgets[class_id] = {
                    'status_label': class_desc_label,
                    'timeout_label': timeout_label,
                    'action_button': action_button,
                    'start_command': (lambda c=cls: start_class_with_instructor_verification(c)),
                    'view_command': (lambda c=cls: view_ongoing_class(c)),
                }
...
def end_class_session(cls, auto=False):
    ...
    if not auto:
        confirm = messagebox.askyesno(
            "End Class",
            f"Are you sure you want to end {class_code}?\n\n"
    ...
    try:
        ...
        if instructor_id is None:
            # Fallback to fetching the class list to find the primary instructor
            classes_response = requests.get(
                f"{SERVER_URL}/classes/api/list",
                headers=HEADERS,
                verify=False,
                timeout=10
            )
        ...
        # Indicate whether this end action is automatic (timeout) so server can record "No Log"
        checkout_data['auto'] = bool(auto)
        checkout_response = requests.post(
            f"{SERVER_URL}/api/checkout/instructor",
            json=checkout_data,
            headers=HEADERS,
            verify=False,
            timeout=10
        ...
        if checkout_response.status_code == 200:
            ...
            # Only show message if not already shown (prevent duplicates)
            if auto and class_id not in _timeout_message_shown:
                ...
                # Always show for manual end class
                try:
                    messagebox.showinfo(title, body)
                except Exception:
                    pass
    ...
    # If we got here, we attempted to end the class (regardless of absent count)
    return True
...
# Create the main window with MPSU branding
root = ctk.CTk()
...
# Make the main window fill the primary monitor and start at the top-left
# corner. We remove the title bar/borders but do NOT force always-on-top,
# so dialogs and child windows (scanner, message boxes) can appear normally.
try:
...
# Place at (0,0) and size to full screen
...
# Try to remove window decorations; if this causes issues on some systems,
# you can comment this out to restore the normal title bar.
try:
