...
try:
    ...
    pass
YEAR_LEVEL_OPTIONS = ['All Year Levels', '1st Year', '2nd Year', '3rd Year', '4th Year']
STUDENT_TABLE_COLUMNS = [('Name', 3), ('Student ID', 2), ('Year Level', 1), ('Face Status', 1), ('Actions', 2)]
REALTIME_REFRESH_MS = 9000
POSE_DISPLACEMENT_PX = 45
ROLL_SEARCH_ANGLES = (-24, -16, -8, 8, 16, 24)
...
class FaceCaptureWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master, student, server_url, api_key, on_success=None):
        ...
        self.auto_capture_interval = 3.0
    ...
    def _build_ui(self):
        ...
        name_label = ctk.CTkLabel(container, text=f"Student: {self.student.get('name') or self.student_id}", font=('Arial', 20, 'bold'), text_color=('#006400', '#90EE90'))
        name_label.pack(pady=(10, 20))
        self.preview_label = ctk.CTkLabel(container, text='Initializing camera...', width=680, height=480, fg_color=('#000000', '#000000'), corner_radius=12)
        self.preview_label.pack(pady=10)
        liveness_panel = ctk.CTkFrame(container, fg_color='transparent')
        liveness_panel.pack(pady=(4, 14))
        self.liveness_label = ctk.CTkLabel(liveness_panel, textvariable=self.liveness_var, font=('Arial', 16, 'bold'), text_color=('#0b5f0b', '#b6f7b6'))
        ...
        self.start_pause_btn = ctk.CTkButton(button_row, text='Start', width=120, height=50, command=self._toggle_start_pause, fg_color=('#28a745', '#218838'), hover_color=('#218838', '#1e7e34'))
        self.start_pause_btn.pack(side='left', padx=8)
        self.restart_btn = ctk.CTkButton(button_row, text='Restart', width=120, height=50, command=self._restart_capture, fg_color=('#6c757d', '#4a4f54'), hover_color=('#545b62', '#3a3f44'))
        self.restart_btn.pack(side='left', padx=8)
        close_btn = ctk.CTkButton(button_row, text='Close', width=120, height=50, command=self._handle_close, fg_color=('#dc3545', '#c82333'), hover_color=('#a71d2a', '#7f151f'))
        close_btn.pack(side='left', padx=8)
        self.status_label = ctk.CTkLabel(container, textvariable=self.status_var, font=('Arial', 14), text_color=('#0b5f0b', '#b6f7b6'))
        self.status_label.pack(pady=(5, 0))
        self.saved_label = ctk.CTkLabel(container, text='Photos saved this session: 0', font=('Arial', 13), text_color=('#0b5f0b', '#b6f7b6'))
    ...
    def _handle_upload_success(self, payload, pose_label=None):
        ...
        if image_path:
            ...
            self.uploaded_image_paths.append({'student_id': self.student_id, 'image_path': image_path, 'server_url': self.server_url})
        ...
        try:
            ...
            if os.path.exists(cache_file):
                try:
                    ...
                    face_data = {'student_embeddings': [], 'student_names': [], 'student_ids': [], 'instructor_embeddings': [], 'instructor_names': [], 'instructor_ids': []}
            else:
                face_data = {'student_embeddings': [], 'student_names': [], 'student_ids': [], 'instructor_embeddings': [], 'instructor_names': [], 'instructor_ids': []}
            ...
            for img_info in self.uploaded_image_paths:
                ...
                try:
                    ...
                    try:
                        embedding_result = DeepFace.represent(img_path=tmp_path, model_name='Facenet512', detector_backend='opencv', enforce_detection=False, align=True)
            ...
            if extracted_count > 0:
                try:
                    ...
                    pass
            elif self.uploaded_image_paths:
                pass
        except Exception as e:
            pass
    ...
    def _describe_pose(self, direction):
        return {'center': 'Face forward and hold still', 'left': 'Tilt your head to the left', 'right': 'Tilt your head to the right', 'up': 'Tilt your chin upward', 'down': 'Tilt your chin downward'}.get(direction, direction)
    ...
    def _detect_center_in_gray(self, gray):
        faces = self.face_detector.detectMultiScale(gray, scaleFactor=1.15, minNeighbors=5, minSize=(90, 90))
        if len(faces) == 0:
            return None
    ...
    def _process_liveness(self, frame):
        if not self.liveness_enabled or not self.liveness_steps or self.liveness_complete or self.auto_capture_triggered:
            return
...
class StudentPhotoViewer(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        header = ctk.CTkLabel(self, text=f"Photos for {self.student.get('name') or self.student_id}", font=('Arial', 24, 'bold'), text_color=('#006400', '#90EE90'))
        header.pack(pady=20)
        ...
        self.placeholder_label = ctk.CTkLabel(self.gallery, textvariable=self.status_var, font=('Arial', 18), text_color=('#0b5f0b', '#b6f7b6'))
        self.placeholder_label.grid(row=0, column=0, columnspan=3, pady=40)
        close_btn = ctk.CTkButton(self, text='Close', width=180, height=48, fg_color=('#dc3545', '#c82333'), hover_color=('#a71d2a', '#7f151f'), command=self.destroy)
    ...
    def _display_images(self, loaded_images, silent=False):
        ...
        if not loaded_images:
            ...
            self.placeholder_label = ctk.CTkLabel(self.gallery, textvariable=self.status_var, font=('Arial', 18), text_color=('#b35c00', '#ffdd99'))
            self.placeholder_label.grid(row=0, column=0, columnspan=3, pady=40)
            return
        for index, (meta, image) in enumerate(loaded_images):
            ...
            ctk.CTkLabel(card, text=timestamp, font=('Arial', 14), text_color=('#0b5f0b', '#b6f7b6')).pack(pady=(0, 10))
    ...
    def _show_error(self, message):
        ...
        self.placeholder_label = ctk.CTkLabel(self.gallery, textvariable=self.status_var, font=('Arial', 18), text_color=('#b91c1c', '#ffb3b3'))
        self.placeholder_label.grid(row=0, column=0, columnspan=3, pady=40)
        self._schedule_auto_refresh()
    ...
    def _schedule_auto_refresh(self):
        ...
        if self.auto_refresh_interval:
            self.auto_refresh_job = self.after(self.auto_refresh_interval, lambda: self._load_images_async(silent=True))
...
class ManualStudentEnrollDialog(ctk.CTkToplevel):
    ...
    def _build_form(self):
        ...
        ctk.CTkLabel(container, text='Enroll Student', font=('Arial', 28, 'bold'), text_color=('#006400', '#90EE90')).pack(pady=(10, 20))
        ...
        self.status_label = ctk.CTkLabel(container, textvariable=self.status_var, font=('Arial', 16), text_color=('#0b5f0b', '#b6f7b6'))
        self.status_label.pack(pady=(18, 8))
        button_row = ctk.CTkFrame(container, fg_color='transparent')
        button_row.pack(pady=10)
        ctk.CTkButton(button_row, text='Cancel', width=140, height=48, fg_color=('#6c757d', '#4a4f54'), hover_color=('#545b62', '#3a3f44'), command=self._close).pack(side='left', padx=10)
        self.submit_btn = ctk.CTkButton(button_row, text='Save Student', width=180, height=52, fg_color=('#0d6efd', '#0a58ca'), hover_color=('#0b5ed7', '#0946a6'), command=self._handle_submit)
    ...
    def _handle_submit(self):
        ...
        if not first or not last or (not student_id):
            ...
            return
        if not re.match('^\\d{2}-\\d{5}$', student_id):
            self.status_var.set('Student ID must use the YY-XXXXX format.')
            return
        payload = {'firstName': first, 'lastName': last, 'id': student_id, 'yearLevel': year_level or YEAR_LEVEL_OPTIONS[1]}
...
class StudentRegistrationWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master=None, instructor_id=None, server_url=None, api_key=None, preselected_class_id=None, preselected_class_label=None, **kwargs):
        ...
        self.after(1000, self._start_cache_update_checking)
        self.focus_force()
        self._enter_fullscreen()
    ...
    def _build_layout(self):
        ...
        ctk.CTkLabel(header, text='Students Registered', font=('Arial', 28, 'bold'), text_color=('#ffffff', '#e6ffe6')).grid(row=0, column=0, padx=30, pady=(20, 4), sticky='w')
        self.class_hint_label = None
        if self.preselected_class_label:
            self.class_hint_label = ctk.CTkLabel(header, text=f'Enrolling for: {self.preselected_class_label}', font=('Arial', 20, 'bold'), text_color=('#fdfde1', '#d4f8d4'))
            self.class_hint_label.grid(row=1, column=0, padx=30, pady=(0, 18), sticky='w')
        ctk.CTkButton(header, text='Back', width=140, height=48, fg_color=('#6c757d', '#4a4f54'), hover_color=('#545b62', '#3a3f44'), command=self._handle_close).grid(row=0, column=2, rowspan=2, padx=30, pady=20, sticky='e')
    ...
    def _open_manual_enroll_dialog(self):
        ...
        self._manual_dialog = ManualStudentEnrollDialog(self, self.server_url, self.headers, target_class=target_class, on_success=self._handle_manual_enroll_success)
    ...
    def _build_filter_row(self, parent):
        ...
        self.class_menu = ctk.CTkOptionMenu(filters, variable=self.class_var, values=list(self.class_options.keys()), command=self._handle_class_change, width=240)
        ...
        self.search_entry = ctk.CTkEntry(filters, textvariable=self.search_var, placeholder_text='Search students by name or ID...', width=260)
        ...
        self.clear_search_btn = ctk.CTkButton(filters, text='Clear', width=80, command=self._clear_search, fg_color=('#dc3545', '#a71d2a'), hover_color=('#b02a37', '#7f1f28'))
        ...
        self.year_filter = ctk.CTkOptionMenu(filters, variable=self.year_filter_var, values=YEAR_LEVEL_OPTIONS, command=lambda _val: self._apply_filters(), width=200)
        self.year_filter.grid(row=1, column=3, padx=10, pady=(0, 12), sticky='we')
        self.count_badge = ctk.CTkLabel(filters, text='Total Students: 0', font=('Arial', 18, 'bold'), text_color=('#006400', '#90EE90'))
    ...
    def _build_table_header(self):
        ...
        for idx, (title, weight) in enumerate(STUDENT_TABLE_COLUMNS):
            ...
            ctk.CTkLabel(self.header_frame, text=title, font=('Arial', 16, 'bold'), text_color=('#ffffff', '#e6ffe6'), anchor='w').grid(row=0, column=idx, padx=12, pady=12, sticky='w')
    ...
    def _handle_manual_enroll_success(self, student_payload, target_class):
        ...
        manual_record = {'id': student_id, 'name': full_name or student_id, 'yearLevel': student_payload.get('yearLevel') or '', 'hasFaceImages': bool(student_payload.get('profileImage')), '_target_class': target_class}
    ...
    def _load_classes_async(self):
        ...
        def task():
            try:
                response = requests.get(f'{self.server_url}/api/instructors/{self.instructor_id}/classes', headers=self.headers, verify=False, timeout=15)
    ...
    def _load_all_students_async(self, silent=False):
        ...
        def task():
            try:
                response = requests.get(f'{self.server_url}/api/instructors/{self.instructor_id}/students', headers=self.headers, verify=False, timeout=20)
    ...
    def _load_class_students_async(self, class_id, silent=False):
        ...
        def task():
            try:
                response = requests.get(f'{self.server_url}/api/instructors/{self.instructor_id}/classes/{class_id}/students', headers=self.headers, verify=False, timeout=20)
    ...
    def _apply_filters(self):
        ...
        if needle:
            students = [student for student in students if needle in (student.get('name') or '').lower() or needle in (student.get('id') or '').lower()]
        if year_filter and year_filter != YEAR_LEVEL_OPTIONS[0]:
            students = [student for student in students if (student.get('yearLevel') or '').lower() == year_filter.lower()]
    ...
    def _update_student_table(self, students):
        ...
        for index, student in enumerate(students, start=1):
            ...
            ctk.CTkLabel(row, text=status_text, text_color=status_color, font=('Arial', 15, 'bold')).grid(row=0, column=3, padx=12, pady=10, sticky='w')
            actions = ctk.CTkFrame(row, fg_color='transparent')
            actions.grid(row=0, column=4, padx=8, pady=6, sticky='e')
            ctk.CTkButton(actions, text='Capture Face', width=120, height=34, command=lambda s=student: self._open_capture_window(s)).pack(side='left', padx=4)
            ctk.CTkButton(actions, text='View Photos', width=120, height=34, fg_color=('#6c757d', '#4a4f54'), hover_color=('#545b62', '#3a3f44'), command=lambda s=student: self._open_photo_viewer(s)).pack(side='left', padx=4)
    ...
    def _try_download_cache_on_startup(self):
        ...
        if not os.path.exists(cache_file):
            ...
            return
        try:
            response = requests.get(f'{self.server_url}/api/face-encodings/meta', headers=self.headers, verify=False, timeout=3)
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
    ...
    def _download_cache_file(self):
        ...
        try:
            response = requests.get(f'{self.server_url}/api/face-encodings', headers=self.headers, verify=False, timeout=30, stream=True)
            if response.status_code != 200:
                return False
    ...
    def _open_capture_window(self, student):
        ...
        if not student_id:
            ...
            return
        FaceCaptureWindow(self, student, self.server_url, self.api_key, on_success=self._handle_capture_success)
    ...
    def _set_status(self, message, level='info'):
        colors = {'info': '#0b5f0b', 'success': '#0d6c0d', 'warning': '#b35c00', 'error': '#b91c1c'}
